...
bool GoAction::Execute(Event event)
{
    ...
    if (param.find("travel") != std::string::npos && param.size() > 7)
    {
        ...
        {
            botAI->TellMasterNoFacing("清除旅途目标");
    ...
    botAI->TellMaster("密语：'go x,y', 'go [game object]', 'go unit' 或 'go position' 我将传送到目标位置。");
    return false;
}

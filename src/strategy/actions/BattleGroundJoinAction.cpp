...
bool BGJoinAction::gatherArenaTeam(ArenaType type)
{
    ...
    {
        LOG_INFO("playerbots", "队伍 #{} <{}> 没有足够的成员进行匹配", arenateam->GetId(),
    ...
    if (!group->Create(bot))
    {
        LOG_INFO("playerbots", "队伍 #{} <{}>: 无法为竞技场队列创建队伍", arenateam->GetId(),
                 arenateam->GetName());
        return false;
    }
    else
    ...
    LOG_INFO("playerbots", "机器人 {} <{}>: <{}>的队长", bot->GetGUID().ToString().c_str(), bot->GetName(),
             arenateam->GetName());
    ...
    for (auto i = begin(members); i != end(members); ++i)
    {
        ...
        LOG_INFO("playerbots", "机器人 {} <{}>: <{}>的成员", member->GetGUID().ToString().c_str(),
                 member->GetName().c_str(), arenateam->GetName().c_str());
    }

    if (group && group->GetMembersCount() >= (uint32)arenateam->GetType())
    {
        LOG_INFO("playerbots", "队伍 #{} <{}> 小队已做好比赛准备", arenateam->GetId(),
                 arenateam->GetName().c_str());
        return true;
    }
    else
    {
        LOG_INFO("playerbots", "队伍 #{} <{}> 小队未做好比赛装备（人数不够）", arenateam->GetId(),
                 arenateam->GetName().c_str());
        group->Disband();
    }
...
bool BGJoinAction::JoinQueue(uint32 type)
{
    ...
    LOG_INFO("playerbots", "机器人 {} {}:{} <{}> 已在 {} {}队列中", bot->GetGUID().ToString().c_str(),
             bot->GetTeamId() == TEAM_ALLIANCE ? "联盟" : "部落", bot->GetLevel(), bot->GetName().c_str(), _bgType.c_str(),
             isRated   ? "积分竞技场"
             : isArena ? "竞技场"
                       : "");
...
bool BGLeaveAction::Execute(Event event)
{
    ...
    if (bot->InBattleground())
    {
        LOG_INFO("playerbots", "机器人 {} {}:{} <{}> 离开 {}", bot->GetGUID().ToString().c_str(),
                 bot->GetTeamId() == TEAM_ALLIANCE ? "联盟" : "部落", bot->GetLevel(), bot->GetName(),
                 isArena ? "竞技场" : "战场");
    ...
    LOG_INFO("playerbots", "机器人 {} {}:{} <{}> 离开了 {} 队列", bot->GetGUID().ToString().c_str(),

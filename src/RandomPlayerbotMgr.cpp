...
void RandomPlayerbotMgr::CheckLfgQueue()
{
    ...
    LOG_DEBUG("playerbots", "LFG 队列检查中...");
    ...
    LOG_DEBUG("playerbots", "LFG 队列检查完毕");
}
...
bool RandomPlayerbotMgr::ProcessBot(uint32 bot)
{
    ...
    if (!isValid)
    {
        ...
        {
            if (player)
                LOG_INFO("playerbots", "Bot #{} {}:{} <{}>: 登出", bot, IsAlliance(player->getRace()) ? "A" : "H",
...
{
    LOG_INFO("playerbots", "{}/{} 机器人 {} 已登录", playerBots.size(), sRandomPlayerbotMgr->GetMaxAllowedBotCount(),
             bot->GetName().c_str());

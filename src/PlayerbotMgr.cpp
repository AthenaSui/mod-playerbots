                                                     ...
                                                     bool admin, uint32 masterAccountId, uint32 masterGuildId)
{
    ...
    if (!addClassBot)
        return "错误：你不能在non-addclass 机器人上使用该命令。";
    ...
    if (GET_PLAYERBOT_AI(bot))
    {
        if (Player* master = GET_PLAYERBOT_AI(bot)->GetMaster())
        {
            ...
            else if (cmd == "init=auto")
            {
                ...
                PlayerbotFactory factory(bot, master->GetLevel(), ITEM_QUALITY_LEGENDARY, mixedGearScore);
                factory.Randomize(false);
                return "成功，GS限制：" + std::to_string(mixedGearScore / PlayerbotAI::GetItemScoreMultiplier(ItemQualities(ITEM_QUALITY_EPIC))) +
                       "(紫装)";
            }
            else if (cmd.starts_with("init=") && sscanf(cmd.c_str(), "init=%d", &gs) != -1)
            {
                ...
                factory.Randomize(false);
                return "成功，GS限制：" + std::to_string(gs / PlayerbotAI::GetItemScoreMultiplier(ItemQualities(ITEM_QUALITY_EPIC))) + "(紫装)";
            }
        }
...
std::vector<std::string> PlayerbotHolder::HandlePlayerbotCommand(char const* args, Player* master)
{
    ...
    if (!*args)
    {
        messages.push_back("用法：list/reload/tweak/self 或 add/addaccount/init/remove 角色名字\n");
        messages.push_back("用法：addclass 职业名字");
        return messages;
    }
    ...
    if (!strcmp(cmd, "addclass"))
    {
        ...
        {
            messages.push_back(
                "addclass：无效职业名字(warrior/paladin/hunter/rogue/priest/shaman/mage/warlock/druid/dk)");
            return messages;
        }
        uint8 claz;
        ...
        messages.push_back("添加机器人失败。没有可用的角色");
        return messages;
    }
